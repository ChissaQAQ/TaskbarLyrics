...
public static class LyricsCache
{
    ...
    private const int SchemaVersion = 8; // 2: 逐字改以 KRC 文本为主体；3: 修首句译文被抢；4: 补 rv 让罗马音有内容 + 一句译文对两行时并回一行；5: 候选改按译文覆盖率择优 + 配对空隙夹逼填空；6: 向后吸附找回「一对二只认出一半」的那半；7: 认出「主歌词整份是译文、原文在 KRC 侧」的投稿；8: 候选择优改「歌名分数优先、覆盖率用比率」（旧缓存里可能正躺着抓错的 Live 版）
