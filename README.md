...
- **自动避让任务栏元素**（可开关，右键菜单可快捷切换）：UIA 枚举任务栏图标/托盘占用区域，
  ...
  空档窄到读不成句子时自动收起封面，把地方全让给文字；开启后任务栏内拖动停用
...
- **文字颜色跟随系统明暗**（默认）：深色任务栏白字、浅色任务栏近黑字，系统换主题自动跟上；
  阴影同步反色（深字配白影、浅字配黑影）。也可切「自定义」固定颜色，设置页备有常用预设色
...
- 所有设置保存在 exe 同目录的 `config.json`，删除即恢复默认
  （那里写不进去时——例如装在 Program Files——自动改存 `%AppData%\TaskbarLyrics\`）
...
- 歌词来源：网易云（含译文，主）→ QQ 音乐 → LRCLIB；
  歌手名宽松匹配（多歌手 / 繁简变体不漏歌），同首歌多版本时先看歌名精确度、
  再挑译文最全的那版（只看译文多少会被长一倍的 Live 版顶掉）
